...
- 6a42ebf: Support owner-only delivery-gallery file uploads through a host-resolved, signed upload policy. Hosts without the optional capability and verifier retain photo/video uploads. Render arbitrary files as file cards and skip image decoding for non-image originals.
...
- 65f0e67: Refresh the admin workspace and document editors with host-derived tactile controls, clearer page and media layouts, and compact mobile filters. Fix listbox pointer selection and quote/contract tab overflow. Add animated mobile dialog sheets with handle-only drag dismissal while preserving focus, scrolling, and existing close controls.
