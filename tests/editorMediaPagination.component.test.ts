...
import type { createEditorMedia } from "../src/lib/editorMedia.svelte";
...
async function render(includeAttachments = false) {
	...
	await tick();
	const media: ReturnType<typeof createEditorMedia> = component.media;
	return { queries, choose, media };
}
