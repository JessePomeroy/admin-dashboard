...
let { members, disabled = false, onChange }: { members: CatalogProductSetMemberDraftForm[]; disabled?: boolean; onChange: (members: CatalogProductSetMemberDraftForm[]) => void } = $props();
</script>
<section aria-labelledby="catalog-set-members-heading">
	...
	{#if members.length === 0}
		<p class="empty"><strong>No set members yet.</strong><span>The imported print-set member links are preserved by the catalog graph.</span></p>
	{:else}
		<ol>
...
<style>
	...
	@media (max-width: 768px) { section { padding: 20px; } .section-heading { flex-wrap: wrap; } dl { grid-template-columns: 1fr; } button { min-height: 44px; } }
</style>
