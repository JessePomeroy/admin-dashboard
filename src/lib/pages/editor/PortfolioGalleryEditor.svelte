...
// This instance owns one gallery. Remote mutations may outlive its teardown,
// but their completions must not touch a newer instance's recovery draft or UI.
let active = true;
...
function clearLocalDraft() {
	if (active && browser) localStorage.removeItem(storageKey);
}
...
async function preview() {
	...
	pendingPreviewWindow = previewWindow;
	previewWindow.opener = null;
	previewing = true;
	saveError = "";
	try {
		if (!(await saveNow()) || !active || !baseRevisionId) {
			previewWindow.close();
			return;
		}
